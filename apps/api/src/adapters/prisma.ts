...
export class PrismaTicketRepository implements TicketRepository {
  ...
  async findById(id: TicketId): Promise<TicketRecord | null> {
    const rec = await this.db.ticket.findUnique({ where: { id } });
  ...
  async update(
    ...
    const rec = await this.db.ticket.update({ where: { id }, data: patch });
    return toTicketRecord(rec);
  }
...
export class PrismaKnowledgeRepository implements KnowledgeRepository {
  ...
  async search(
    ...
    // DB 후보를 카테고리로 좁혀 가져온 뒤, 앱단에서 키워드 점수 매칭.
  ...
  async list(
    ...
    const rows = await this.db.knowledgeItem.findMany({ where: { category } });
...
export class PrismaCallbackRepository implements CallbackRepository {
  ...
  async findById(id: CallbackId): Promise<CallbackRecord | null> {
    const rec = await this.db.callbackRequest.findUnique({ where: { id } });
