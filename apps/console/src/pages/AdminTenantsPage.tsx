/**
 * 총괄관리자 — 콘솔로 통합된 사업장 관리 화면(구 apps/admin TenantsAdminPage).
 *
 * 진입: /login 에서 platform_admin 계정으로 로그인 → /admin.
 * 별도 관리자 앱/배포 없이 콜비 랜딩(/)이 유일한 진입점이고, 지정된 계정으로
 * 로그인했을 때만 이 화면이 열린다(가드는 RequirePlatformAdmin).
 *
 * 기능: 승인 큐(승인·반려 + 업종 팩 자동 적용 결과 토스트), 전체 사업장 조회,
 * 신규 테넌트+관리자 계정 직접 생성(영업 직접 온보딩).
 */
import { useEffect, useMemo, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import type { TenantReviewResult, TenantStatus } from "@colli/contracts";
import {
  TENANT_STATUS_LABELS,
  TENANT_PLAN_METAS,
  isPhoneNumberAssigned,
} from "@colli/contracts";
import { adminApi, type TenantAdminListItem } from "../api/adminApi";
import { apiErrorCode } from "../api/client";
import { logoutSession } from "../lib/session";
import { useSession } from "../lib/useSession";
import { PageHeader } from "../components/PageHeader";
import { Badge, STATUS_BADGE_TONES } from "../components/Badge";
import { Loading, ErrorBlock, EmptyBlock } from "../components/StateBlock";
import { Logo, btnPrimary, btnSecondary, btnSmall } from "../components/ui";
...
const fieldCls =
  "mt-1 block w-full rounded-lg border border-ink-200 px-3 py-2 text-sm focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100";
...
export function AdminTenantsPage() {
  ...
  return (
    <div className="min-h-screen bg-ink-50">
      {/* 콘솔 통합 관리자 헤더 — AppShell(사업장 스코프) 대신 자체 미니 셸 */}
      <header className="border-b border-ink-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-6 py-3.5">
          <div className="flex items-center gap-3">
            <Logo />
            <Badge tone="bg-ink-900 text-white">총괄관리자</Badge>
          </div>
          <div className="flex items-center gap-3 text-sm text-ink-500">
            {session ? <span>{session.account.email}</span> : <span>데모 모드</span>}
            <button onClick={onLogout} className={btnSmall}>
              로그아웃
            </button>
          </div>
        </div>
                  ...
                  <tbody className="divide-y divide-ink-100">
                    {filtered.map((t) => (
                      <tr key={String(t.tenantId)} className="hover:bg-ink-50">
                        ...
                        <td className="px-4 py-3">
                          <Badge tone={STATUS_BADGE_TONES[t.status as TenantStatus]}>
                            {TENANT_STATUS_LABELS[t.status as TenantStatus]}
                          </Badge>
                        </td>
                        <td className="px-4 py-3 text-ink-600">
                          {TENANT_PLAN_METAS[t.plan].name}
        ...
        <section className="mb-6 rounded-xl border border-ink-200 bg-white p-4">
          ...
          >
            ...
            <div className="sm:col-span-2">
              <button type="submit" disabled={!canSubmit || create.isPending} className={btnPrimary}>
          ...
          {result ? (
            <p className="mt-3 rounded-lg bg-success-50 px-3 py-2 text-sm text-success-700">
              계정이 생성되었습니다. <strong>{result.email}</strong> 로 로그인하면 사업장
              콘솔이 열려요.
            </p>
          ) : null}
        </section>
        ...
        {approveTarget ? (
          <ApproveModal
            ...
            onSuccess={(reviewResult) => {
              ...
              if (reviewResult.industryTemplate) {
                showToast(
                  `승인 완료 — 070 배정됨 · ${reviewResult.industryTemplate.packTitle} 자동 적용(문의 유형 ${reviewResult.industryTemplate.createdIntentKeys.length}개)`,
                );
              } else if (reviewResult.industryTemplateError) {
                showToast(
                  `승인 완료 — 070 배정됨 · 업종 팩 자동 적용 실패(콘솔에서 수동 적용 가능): ${reviewResult.industryTemplateError}`,
                );
              } else {
...
function ApproveModal({
  ...
  const approve = useMutation({
    mutationFn: () =>
      adminApi.approveTenant(String(tenant.tenantId), { phoneNumber: phoneNumber.trim() }),
    onSuccess: (reviewResult) => onSuccess(reviewResult),
    onError: (err) => {
    ...
    <ModalShell title={`${tenant.name} 신청 승인`} onClose={onClose}>
      <form onSubmit={submit} noValidate>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="submit" disabled={approve.isPending} className={btnPrimary}>
...
function RejectModal({
  ...
  const reject = useMutation({
    mutationFn: () =>
      adminApi.rejectTenant(String(tenant.tenantId), { reason: reason.trim() }),
    onSuccess,
    onError: (err) => {
      setFieldError(err instanceof Error ? err.message : String(err));
    },
