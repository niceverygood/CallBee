 ...
 *
 * fixture 모드는 RequireAuth 가 항상 통과시키므로 /login 을 거치지 않고
 * FIXTURE_TENANT_ID(BoBi) 로 바로 진입한다. fetch 모드는 로그인 후 세션에
 * 저장된 account.tenantId 가 "현재 테넌트"가 된다(getCurrentTenantId()).
 */
export function App() {
      ...
      <Route element={<RequireAuth />}>
        <Route element={<AppShell />}>
          ...
          <Route
            ...
            element={
              <Navigate
                to={currentTenantId ? `/tenants/${currentTenantId}/settings/agent` : "/onboarding"}
                replace
              />
            }
          />
