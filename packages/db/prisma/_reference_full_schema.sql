-- 참고용 SQL — 실행하지 않았음(실인프라 접속 금지 지시에 따름).
-- `prisma migrate diff --from-empty --to-schema-datamodel prisma/schema.prisma --script`
-- 로 DB 접속 없이 오프라인 계산한 "빈 DB → 현재 schema.prisma" 전체 SQL이다.
-- 목적: schema.prisma 가 문법적으로 완전하고 유효한 마이그레이션을 생성할 수
-- 있음을 사전 검증하기 위함(신규 4테이블 CreateTable 포함). 실제 라이브 DB
-- 적용은 docs/tenant-platform-architecture.md §1.4/§6 순서(신규 테이블 생성 →
-- 기존 7테이블 tenantId nullable 추가 → 백필 → NOT NULL 전환 → enum→text 전환,
-- 이 4단계는 각각 별도 `prisma migrate dev`로 나눠 실행해야 함 — 이 파일처럼
-- 한 번에 밀어넣으면 기존 테이블에 이미 NOT NULL tenantId 를 강제하게 되어
-- 데이터가 있는 라이브 DB에서 실패한다)를 그대로 따라 Orchestrator/운영자가
-- 실행한다. 이 파일은 실행 스크립트가 아니라 검증 기록이다.
