/**
 * BoBi 상담 system prompt 빌더 (Worker B).
 *
 ...
 */
import { INTENTS, INTENT_LABELS, type Intent } from "@colli/contracts";
...
/** 의도 카탈로그를 프롬프트에 넣기 좋은 한 줄 목록으로 렌더 */
...
export function buildSystemPrompt(options: SystemPromptOptions = {}): string {
  ...
  // ── 통화 초입 고지 (GUARDRAIL #3) ────────────────────────────
  if (!consentDone) {
    sections.push(
      [
        `# 통화 시작 (필수)`,
        `통화 초입에 다음을 반드시 먼저 고지하고 동의를 확인합니다:`,
